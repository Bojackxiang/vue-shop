...
export default {
  ...
  methods: {
    getCartList() {
      // 初始化数据
      // 初始化列表
      this.axios.get("/carts/").then(resp => {
    ...
    toggleAll() {
      ...
      this.axios.put(path).then(toggleSelectResp => {
        this.renderData(toggleSelectResp); // 只更新list是没有用的，要做哟个render data
      });
    ...
    updateCart(item, operation) {
      ...
      this.axios
        ...
        .then(updateCartResp => {
          this.renderData(updateCartResp);
        });
    },
